...
const AddToPlaylistModal = ({closeModal,addSongToPlaylist}) => {
    ...
    return(
        <div className="absolute w-screen h-screen bg-opacity-50 bg-black flex justify-center items-center" onClick={closeModal}>
            <div className="bg-app-black w-1/3 rounded-md p-4" onClick={(e) => {e.stopPropagation()}}>
                ...
                <div className="space-y-4 flex flex-col justify-center items-center">
                    {myPlaylists.map(item => {
                        return (
                            <PlaylistListComponent info={item} addSongToPlaylist={addSongToPlaylist}/>
                        );
                    })}
...
const PlaylistListComponent = ({info, addSongToPlaylist}) => {
    return (
        <div className="bg-app-black w-full flex items-center space-x-4 hover:bg-gray-400 hover:bg-opacity-20 cursor-pointer p-3" onClick={() => {addSongToPlaylist(info._id)}}>
            <div>
                <img src={info.thumbnail} alt="playlistthumbnail" className="h-14 w-14 rounded"/>
            </div>
