...
const Library = () => {
    ...
    return(
        <LoggedInContainer curActiveScreen={"library"}>
            ...
            <div className="py-5 grid gap-4 grid-cols-5 cursor-pointer">
                {myPlaylists.map((item) => {
                    return (
                    <Card  key={JSON.stringify(item)} title={item.name} description="" imgUrl={item.thumbnail} playlistId={item._id}/>);
                })}
            </div>
